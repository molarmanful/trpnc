...
sto=_=>{
  ...
  O.write('\033c'+stack.map((a,b)=>b+'\t'+a).reverse().join`\n`)
}
...
process.stdin.on('keypress',key=(a='',b='')=>{
  ...
  //ex mode
  :a==';'?
    (stack[0]?O.write('\n'):stack.shift(),process.stdin.pause(),O.green().write('\r\t\r'),exec(I(';')||''),process.stdin.resume(),stack.unshift(''))
